    ...
    <div className="bg-spice-cream min-h-screen py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-50 text-spice-red text-xs font-bold uppercase tracking-wider mb-4">
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-3xl p-6 md:p-8 border border-spice-brown/15 shadow-xl mb-16">
          ...
          <form onSubmit={handleVerify} className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              />
              <Search className="w-4 h-4 text-ink-500 absolute left-3.5 top-4" />
            </div>
            <button
              type="submit"
              disabled={loading}
          ...
          {/* Error display */}
          {errorMsg && (
            <div className="mt-6 p-4 bg-brand-50 border border-brand-200 rounded-2xl flex items-center gap-3 text-spice-red text-xs">
          ...
          {batchResult && (
            <div className="mt-8 pt-8 border-t border-spice-brown/15 space-y-6">
              <div className="flex items-center justify-between bg-spice-brown p-4 rounded-2xl border border-spice-brown">
                <div className="flex items-center gap-3">
                  <CheckCircle2 className="w-6 h-6 text-spice-turmeric" />
                  <div>
                    <span className="font-serif font-bold text-sm text-spice-cream block">
                      Authentic & Lab Verified Batch
                    </span>
                    <span className="text-[11px] text-spice-beige/80">
                      Certificate #{batchResult.qualityReport.certificateNumber}
                    </span>
                  </div>
                </div>
                <span className="bg-spice-red text-white font-bold text-[10px] px-2.5 py-1 rounded-full uppercase">
                  VERIFIED
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-spice-brown">
                <div className="p-3 bg-spice-beige/40 rounded-xl border border-spice-brown/10">
                  <span className="text-ink-500 block text-[10px]">Product Name</span>
                  <span className="font-serif font-bold text-sm">{batchResult.productName}</span>
                </div>
                <div className="p-3 bg-spice-beige/40 rounded-xl border border-spice-brown/10">
                  <span className="text-ink-500 block text-[10px]">Batch Number</span>
                  <span className="font-mono font-bold text-sm text-spice-red">{batchResult.batchNumber}</span>
                </div>
                <div className="p-3 bg-spice-beige/40 rounded-xl border border-spice-brown/10">
                  <span className="text-ink-500 block text-[10px]">Manufacturing Date</span>
                  <span className="font-bold">{new Date(batchResult.mfgDate).toLocaleDateString()}</span>
                </div>
                <div className="p-3 bg-spice-beige/40 rounded-xl border border-spice-brown/10">
                  <span className="text-ink-500 block text-[10px]">Expiry Date</span>
              ...
              <div className="bg-white p-5 rounded-2xl border border-spice-brown/15 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                  <div className="p-3 bg-spice-cream rounded-xl">
                    <span className="text-ink-500 text-[10px] block">Purity & Curcumin</span>
                    <span className="font-bold text-spice-brown">{batchResult.qualityReport.purityScore}</span>
                  </div>
                  <div className="p-3 bg-spice-cream rounded-xl">
                    <span className="text-ink-500 text-[10px] block">Moisture Content</span>
                    <span className="font-bold text-spice-brown">{batchResult.qualityReport.moistureLevel}</span>
                  </div>
                  <div className="p-3 bg-spice-cream rounded-xl">
                    <span className="text-ink-500 text-[10px] block">Microbial Safety</span>
          ...
          {[
            ...
            return (
              <div key={i} className="bg-white p-8 rounded-3xl border border-spice-brown/10 shadow-sm text-center">
                <div className="w-14 h-14 rounded-2xl bg-brand-50 text-spice-red flex items-center justify-center mx-auto mb-4">
