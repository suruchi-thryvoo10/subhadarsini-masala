    ...
    <div className="bg-spice-cream min-h-screen p-6 md:p-10 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-3xl border border-spice-brown/10 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-spice-brown/60 block">Total Revenue</span>
            <span className="font-serif font-bold text-2xl text-spice-brown">₹{stats?.totalRevenue || 148500}</span>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-spice-saffron/15 text-spice-saffron flex items-center justify-center font-bold">
        ...
        <div className="bg-white p-6 rounded-3xl border border-spice-brown/10 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-spice-brown/60 block">Total Orders</span>
        ...
        <div className="bg-white p-6 rounded-3xl border border-spice-brown/10 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-spice-brown/60 block">Registered Customers</span>
        ...
        <div className="bg-white p-6 rounded-3xl border border-spice-brown/10 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs text-spice-brown/60 block">Catalogue Items</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-3xl border border-spice-brown/10 shadow-sm space-y-4">
          ...
          {stats?.lowStockProducts?.length === 0 ? (
            <p className="text-xs text-spice-brown/60">All inventory stock levels are healthy.</p>
          ) : (
            <div className="space-y-3">
              {stats?.lowStockProducts?.map((p: any) => (
                <div key={p._id} className="p-3 bg-amber-50 rounded-2xl border border-amber-200 flex items-center justify-between text-xs">
                  <span className="font-bold text-amber-900">{p.name}</span>
                  <span className="font-bold text-amber-700 bg-amber-200 px-2 py-0.5 rounded">
            ...
            {stats?.recentOrders?.map((ord: any) => (
              <div key={ord._id} className="p-3 bg-spice-cream rounded-2xl border border-spice-brown/10 flex items-center justify-between text-xs">
                <div>
                  ...
                  <span className="text-[10px] text-spice-brown/60">{ord.shippingAddress?.name}</span>
                </div>
