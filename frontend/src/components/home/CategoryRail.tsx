 ...
 *
 * Cards are sized so roughly four sit in a desktop viewport at once. The rail
 * drifts slowly by itself and can be swiped, trackpad-scrolled or
 * mouse-dragged; see useAutoScrollRail.
 */
export const CategoryRail: React.FC<CategoryRailProps> = ({ categories, loading }) => {
  const t = useT();
  const categoryName = useCategoryName();
  const { scrollerRef, trackRef } = useAutoScrollRail(loading ? 0 : categories.length);
  if (loading) {
    return (
  ...
  return (
    <div className="relative">
      ...
      <div ref={scrollerRef} className="rail-scroller overflow-x-auto overflow-y-hidden scrollbar-none">
        <div ref={trackRef} className="flex gap-3 sm:gap-6 w-max py-2">
          {track.map((cat, idx) => (
            <Link
              key={`${cat._id}-${idx}`}
