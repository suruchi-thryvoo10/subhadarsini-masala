...
export const autoSeedIfEmpty = async () => {
    try {
        ...
        if (productCount > 0) {
            console.log(`[AutoSeed] Database already contains ${productCount} products. Skipping initial catalog seed.`);
            return;
        }
        console.log('🌱 [AutoSeed] Initializing Subhadarshini Demo Products & Catalog...');
        // 1. Seed Users
        ...
        const gourmetCat = categories.find(c => c.slug === 'gourmet-seasonings')._id;
        // 3. Seed 22 Authentic Subhadarshini Products
        ...
        // 4. Seed Quality Traceability Batches
        await Batch.create([
            {
        ...
        await Recipe.create([
            {
                ...
                requiredProducts: [products[0]._id, products[1]._id, products[6]._id],
                isFeatured: true
            }
        ]);
        // 6. Seed Dealers
        ...
        console.log('🎉 [AutoSeed] 22 Subhadarshini demo products & collections successfully initialized!');
    }
    catch (error) {
